...
function StatCard({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="luxury-glass-card p-5">
      <p className="text-[11px] text-neutral-400 uppercase font-bold tracking-wider mb-1">{label}</p>
      <p className="text-2xl font-serif font-black text-white">{value}</p>
      {sub && <p className="text-xs text-[#DFBA73] mt-1">{sub}</p>}
    </div>
  );
}
